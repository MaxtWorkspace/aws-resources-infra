...
const logger = winston.loggers
  .get('sdk-logger')
  .child({ service: 'Route53, Cognito Identity Provider' });
logger.debug(
  'Post-process - Create Route53 alias CNAME record for cognito user pool domain',
);
...
try {
  ...
  pollWithPrerequisites(
    [CreateCognitoAliasRecord],
    verifyCreated,
    config.polling.checkInterval * 1000,
    config.polling.timeOut * 1000,
  );
  pollUntilDone(
    UploadHostedUICustomization,
    config.polling.checkInterval * 1000,
    config.polling.timeOut * 1000,
  );
} catch (err) {
...
async function CreateCognitoAliasRecord() {
  ...
  await poll(
    async () => {
      const response = await getCognitoDomainDescribe(
        clientCognito,
        config.domain.domainName,
      );
      ...
      if (!status) {
        throw new Error(
          `Invalid Domain Status - status is undefined for domain: ${config.domain.domainName}, it must be active to retreive Alias target`,
        );
      }
  ...
  await sendChangeRecord(
    clientRoute53,
    config.domain.hostedZoneId,
    'CREATE',
    'Create the cognito domain alias CNAME record before CDK Deploy',
    record,
  );
}
...
async function verifyCreated() {
  ...
  if (records.length === 1) {
    logger.info(
      'Record CREATED Successfully - cognito domain alias recod is created',
    );
    return true;
  }
...
async function UploadHostedUICustomization() {
  const cognitoDomainResponse = await getCognitoDomainDescribe(
    clientCognito,
    config.domain.domainName,
  );
  ...
  const response = await setUserPoolUICustomization(
    clientCognito,
    css,
    logo,
    userPoolId,
  ...
  if (css != returnedCss) {
    throw new Error(
      'UI Customization failed - uploaded css and returned css mismatch, please manually upload',
    );
  }

  if (!lastModified) {
    logger.warn(
      'Existing UI Customization - css updated, but did not verify image matched with uploaded image(verify manually if desired)',
    );
    return;
  }
  ...
  if (elapsed > 10 * 1000) {
    throw new Error(
      'UI Customization failed - last modified date more than 10 seconds, please manually upload',
    );
  }

  if (elapsed > 1000) {
    logger.warn(
      'UI Customization Might Fail - last modified date more than 1 second, please validate and manually upload',
    );
  }

  logger.warn(
    `Uploaded Successfully - successfully uploaded css and logo to cognito hosted UI under domain: ${config.domain.domainName}`,
  );
}
